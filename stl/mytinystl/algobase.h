...
const T& max(const T& lhs, const T& rhs, const Compare& comp)
{
...
// 重载版本使用函数对象 comp 代替比较操作
template <typename T, typename Compare>
const T& min(const T& lhs, const T& rhs, const Compare& comp)
{
...
BidirectionalIter2
unchecked_copy_backward_cat(BidirectionalIter1 first, BidirectionalIter1 last, BidirectionalIter2 result, mystl::bidirectional_iterator_tag)
{
...
BidirectionalIter unchecked_copy_backward_cat(RandomIter first, RandomIter last, BidirectionalIter result, mystl::random_access_iterator_tag)
{
...
template <class BidirectionalIter1, class BidirectionalIter2>
BidirectionalIter1 unchecked_move_backward_cat(BidirectionalIter1 first, BidirectionalIter1 last, BidirectionalIter2 result, mystl::bidirectional_iterator_tag)
{
...
template <class RandomIter, class BidirectionalIter>
BidirectionalIter unchecked_move_backward_cat(RandomIter first, RandomIter last, BidirectionalIter result, mystl::random_access_iterator_tag)
{
    for(auto n = last - first; n > 0; --n)
    {
...
bool equal(InputIter1 first1, InputIter1 last1, InputIter2 first2, BinaryPredicate binary_pred)
{
    for(; first1 != last1; ++first1, ++first2)
    {
...
&& !std::is_same<Tp, bool>::value && std::is_integral<Up>::value && sizeof(Up) == 1, Tp*>::type
...
template <class OutputIter, class Size, class Up>
OutputIter fill_n(OutputIter first, Size n, Up value)
{

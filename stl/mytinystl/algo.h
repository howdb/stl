...
ForwardIter search_n(ForwardIter first, ForwardIter last, Size n, const T& value)
{
    ...
    {   
        ...
        {
            first = mystl::find(it, last, value);
        }
    }
    return last;
}
...
ForwardIter1 find_end_dispatch(ForwardIter1 first1, ForwardIter1 last1,
                                ForwardIter2 first2, ForwardIter2 last2, forward_iterator_tag, forward_iterator_tag)
{
                                        ...
                                        BidirectionalIter2 first2, BidirectionalIter2 last2, bidirectional_iterator_tag, bidirectional_iterator_tag)
{
    ...
    else
    {
        ...
        mystl::advance(rresult, -mystl::distance(first2, last2));
        return result;
    }
}
...
ForwardIter1 find_end_dispatch(ForwardIter1 first1, ForwardIter1 last1, ForwardIter2 first2, ForwardIter2 last2,
                                Compared comp, forward_iterator_tag, forward_iterator_tag)
{
    if(first2 == last2)return last1;
    else
    {
...
BidirectionalIter1 find_end_dispatch(BidirectionalIter1 first1, BidirectionalIter1 last1, BidirectionalIter2 first2, BidirectionalIter2 last2,
                                        Compared comp, bidirectional_iterator_tag, bidirectional_iterator_tag)
{
    typedef reverse_iterator<BidirectionalIter1> reviter1;
    typedef reverse_iterator<BidirectionalIter2> reviter2;
    reviter1 rlast1(first1);
...
template <typename ForwardIter1, typename ForwardIter2, typename Compared>
ForwardIter1 find_end(ForwardIter1 first1, ForwardIter1 last1, ForwardIter2 first2, ForwardIter2 last2, Compared comp)
{
    typedef typename iterator_traits<ForwardIter1>::iterator_category Category1;
    typedef typename iterator_traits<ForwardIter2>::iterator_category Category2;
...
ForwardIter lbound_dispatch(ForwardIter first, ForwardIter last, const T& value, Compared comp, forward_iterator_tag)
{
    auto len = mystl::distance(first, last);
    auto half = len;
    ForwardIter middle;
...
RandomIter lbound_dispatch(RandomIter first, RandomIter last, const T& value, Compared comp, random_access_iterator_tag)
{
    auto len = last - first;
    auto half = len;
    RandomIter middle;
...
{
    return mystl::lbound_dispatch(first, last, value, comp, iterator_category(first));
}
...
// ubound_dispatch 的 forward_iterator_tag 版本
template <typename ForwardIter, typename T, typename Compared>
ForwardIter ubound_dispatch(ForwardIter first, ForwardIter last, const T& value, Compared comp, forward_iterator_tag)
{
    auto len = mystl::distance(first, last);
    auto half = len;
    ForwardIter middle;
...
// ubound_dispatch 的 random_access_iterator_tag 版本
template <typename RandomIter, typename T, typename Compared>
RandomIter ubound_dispatch(RandomIter first, RandomIter last, const T& value, Compared comp, random_access_iterator_tag)
{
    auto len = last - first;
    auto half = len;
    RandomIter middle;
...
{
    return mystl::ubound_dispatch(first, last, value, comp, iterator_category(first));
}
...
erange_dispatch(ForwardIter first, ForwardIter last, const T& value, forward_iterator_tag)
{
    ...
    while(len > 0)
    {
        ...
        else
        {
            ...
            return mystl::make_pair<ForwardIter, ForwardIter>(left, right);
        }
    }
    return mystl::make_pair<ForwardIter, ForwardIter>(last, last);
}
...
erange_dispatch(RandomIter first, RandomIter last, const T& value, random_access_iterator_tag)
{
    ...
    while(len > 0)
    {
        ...
        else
        {
            ...
            return mystl::make_pair<RandomIter, RandomIter>(left, right);
        }
    }
    return mystl::make_pair<RandomIter, RandomIter>(last, last);
}
...
template <typename ForwardIter, typename T, typename Compared>
mystl::pair<ForwardIter, ForwardIter>
erange_dispatch(ForwardIter first, ForwardIter last, const T& value, Compared comp, forward_iterator_tag)
{
    auto len = mystl::distance(first, last);
    auto half = len;
    ForwardIter middle, left, right;
...
template <typename RandomIter, typename T, typename Compared>
mystl::pair<RandomIter, RandomIter>
erange_dispatch(RandomIter first, RandomIter last, const T& value, Compared comp, random_access_iterator_tag)
{
    auto len = last - first;
    auto half = len;
    RandomIter middle, left, right;
...
{
    return mystl::erange_dispatch(first, last, value, comp, iterator_category(first));
}
...
void generate(ForwardIter first, Size n, Generator gen)
{
    for(; n > 0; --n, ++first)
    {
        *first = gen();
...
/*****************************************************************************************/
template <typename ForwardIter>
bool is_sort(ForwardIter first, ForwardIter last)
{
...
template <typename ForwardIter, typename Compared>
bool is_sort(ForwardIter first, ForwardIter last, Compared comp)
{
    if(first == last)return true;
    auto next = first;
    ++next;
...
{
    first = mystl::find(first, last, unary_pred);
    auto next = first;
...
OutputIter
replace_copy_if(InputIter first, InputIter last, OutputIter result, UnaryPredicate unary_pred, const T& new_value)
{
    for(; first != last; ++first, ++result)
    {
...
void random_shuffle(RandomIter first, RandomIter last)
{
    ...
    {
        mystl::iter_swap(it, first + (rand() % (it - first + 1)));
    }
}
...
void random_shuffle(RandomIter first, RandomIter last, RandomNumberGenerator& rand)
{
    ...
    for(auto it = first + 1; it != last; ++it)
    {
        mystl::iter_swap(it, first + (rand(it - first + 1) % len));
    }
}
...
BidirectionalIter rotate_dispatch(BidirectionalIter first, BidirectionalIter middle, BidirectionalIter last, bidirectional_iterator_tag)
{
    ...
    while(first != middle && middle != last)mystl::swap(*first++, *--last);
    if(first == middle){
        mystl::reverse_dispatch(middle, last, bidirectional_iterator_tag());
        return last;
    }
...
// 求最大公因子
template <typename EuclideanRingElem>
EuclideanRingElem rgcd(EuclideanRingElem m, EuclideanRingElem n)
{
    while(n != 0)
    {
        auto t = m % n;
...
void merge_without_buffer(BidirectionalIter first, BidirectionalIter middle, BidirectionalIter last,
                            Distance len1, Distance len2)
{
    if(len1 == 0 || len2 == 0)return;
    if(len1 + len2 == 2)
    {
        if(*middle < *first)mystl::iter_swap(first, middle);
        return;
    }
...
BidirectionalIter1 merge_backward(BidirectionalIter1 first1, BidirectionalIter1 last1,
                                    BidirectionalIter2 first2, BidirectionalIter2 last2, BidirectionalIter1 result)
{
...
BidirectionalIter1 rotate_adaptive(BidirectionalIter1 first, BidirectionalIter1 middle, BidirectionalIter1 last,
...
void merge_adaptive(BidirectionalIter first, BidirectionalIter middle, BidirectionalIter last,
                            ...
                            Distance len1, Distance len2, Compared comp)
{
    ...
    if(len1 + len2 == 2)
    {
        if(comp(*middle, *first))mystl::iter_swap(first, middle);
        return;
    }
    auto first_cut = first;
    auto second_cut = middle;
...
template <typename BidirectionalIter1, typename BidirectionalIter2, typename Compared>
BidirectionalIter1 merge_backward(BidirectionalIter1 first1, BidirectionalIter1 last1,
                                    BidirectionalIter2 first2, BidirectionalIter2 last2, BidirectionalIter1 result, Compared comp)
{
    if(first1 == last1)return mystl::copy_backward(first2, last2, result);
    if(first2 == last2)return mystl::copy_backward(first1, last1, result);
    --last1;
...
{
    return psort_copy_aux(first, last, result_first, result_last, distance_type(result_first));
}

// 重载版本使用函数对象 comp 代替比较操作
template <typename InputIter, typename RandomIter, typename Distance, typename Compared>
RandomIter psort_copy_aux(InputIter first, InputIter last, RandomIter result_first, RandomIter result_last, Distance*, Compared comp)
{
    if(result_first == result_last)return result_last;
    auto result_iter = result_first;
    while(first != last && result_iter != result_last)
...
template <typename InputIter, typename RandomIter, typename Distance, typename Compared>
RandomIter partial_sort_copy(InputIter first, InputIter last, RandomIter result_first, RandomIter result_last, Compared comp)
{
    return psort_copy_aux(first, last, result_first, result_last, distance_type(result_first), comp);
}
...
BidirectionalIter partition(BidirectionalIter frist, BidirectionalIter last, UnaryPredicate unary_pred)
{
    while(true)
    {
        while(frist != last && unary_pred(*frist))++frist;
        if(frist == last)break;
        --last;
        while(frist != last && !unary_pred(*last))--last;
        if(frist == last)break;
        mystl::iter_swap(frist, last);
        ++frist;
    }
    return frist;
}
...
void final_insertion_sort(RandomIter first, RandomIter last)
{
    ...
    {
        mystl::unchecked_insertion_sort(first, last);
    }
}
...
// 插入排序辅助函数 unchecked_linear_insert
template <typename RandomIter, typename T, class Compared>
...
void final_insertion_sort(RandomIter first, RandomIter last, Compared comp)
{
    ...
    {
        mystl::unchecked_insertion_sort(first, last, comp);
    }
}
...
void nth_element(RandomIter first, RandomIter nth, RandomIter last)
{
    ...
    while(last - first > 3)
    {
        ...
        if(cut <= nth)first = cut;  // 对右段进行分割
        else last = cut;  // 对左段进行分割
    }
...
void nth_element(RandomIter first, RandomIter nth, RandomIter last, Compared comp)
{
    ...
    {
        auto cut = mystl::unchecked_partition(first, last,
                    mystl::median(*first, *(first + (last - first) / 2), *(last - 1), comp), comp);
        if(cut <= nth)first = cut;  // 对右段进行分割
        else last = cut;  // 对左段进行分割
    }
...
OutputIter unique_copy_dispatch(InputIter first, InputIter last, OutputIter result, Compared comp, output_iterator_tag)
{
    auto value = *first;
    *result = value;
    while(++first != last)
...
ForwardIter unique_copy_dispatch(InputIter first, InputIter last, ForwardIter result, Compared comp, forward_iterator_tag)
{
    *result = *first;
    while(++first != last)
    {
...
{
    if(first == last)return result;
    return mystl::unique_copy_dispatch(first, last, result, comp, iterator_category(result));
}

...
public:
    ...
    typename std::enable_if<mystl::is_input_iterator<InputIterator>::value, int>::type = 0>
...
public:
    ...
    template <class InputIterator, typename std::enable_if<mystl::is_input_iterator<InputIterator>::value, int>::type = 0>
...
private:
    ...
    template <class InputIterator>
    void copy_init(InputIterator first, InputIterator last, input_iterator_tag);
    
    template <class ForwardIterator>
    void copy_init(ForwardIterator first, ForwardIterator last, forward_iterator_tag);
    ...
    template<class ForwardIterator>
    void copy_insert(iterator, ForwardIterator, ForwardIterator, size_type);

    template<class InputIterator>
    void insert_dispatch(iterator, InputIterator, InputIterator, input_iterator_tag);

    template<class ForwardIterator>
    void insert_dispatch(iterator, ForwardIterator, ForwardIterator, forward_iterator_tag);
...
void deque<T>::resize(size_type new_size, const value_type& value)
{
    ...
    if(new_size < len)
    {
        erase(begin_ + new_size, end_);  // 容器只会变大不会变小
    }
    else
    {
...
void deque<T>::emplace_back(Args&&... args)
{
    ...
    else
    {
        ...
        // require_capacity操作后end_已经被更新了，后面的new_buffer是在这一步才创建的
        data_allocator::construct(end_.cur, mystl::forward<Args>(args)...);
        ++end_;
    }
}
...
{
    const size_type nNode = nElem / buffer_size + 1;  // 需要分配的结点个数
...
template <class T>
template <class InputIterator>
void deque<T>::copy_init(InputIterator first, InputIterator last, input_iterator_tag)
{
...
template <class T>
template <class ForwardIterator>
void deque<T>::copy_init(ForwardIterator first, ForwardIterator last, forward_iterator_tag)
{
    const size_type n = mystl::distance(first, last);
    map_init(n);
    for(auto cur = begin_.node; cur < end_.node; ++cur)
...
template <class T>
template <class InputIterator>
void deque<T>::copy_assign(InputIterator first, InputIterator last, input_iterator_tag)
{
    auto first1 = begin();
    auto last1 = end();
...
template <class T>
template <class ForwardIterator>
void deque<T>::copy_assign(ForwardIterator first, ForwardIterator last, forward_iterator_tag)
{
...
typename deque<T>::iterator deque<T>::insert_aux(iterator pos, Args&&... args)
{
    ...
    {   // 在前半段插入
        ...
        auto position = pos;
        ++position;
        mystl::copy(front2, position, front1);
    }
    else
    {   // 在后半段插入
        emplace_back(back());
    ...
    if(elems_before < (len / 2))
    {
        ...
        try
        {
            ...
            else
            {
                ...
                mystl::uninitialized_copy(first, mid, mystl::uninitialized_copy(begin_, pos, new_begin));
                begin_ = new_begin;
    ...
    else
    {
        ...
        try
        {
            ...
            else
            {
                ...
                mystl::uninitialized_copy(pos, end_, mystl::uninitialized_copy(mid, last, end_));
                end_ = new_end;
...
template <class T>
template <class InputIterator>
void deque<T>::insert_dispatch(iterator pos, InputIterator first, InputIterator last, input_iterator_tag)
{
...
template <class T>
template <class ForwardIterator>
void deque<T>::insert_dispatch(iterator pos, ForwardIterator first, ForwardIterator last, forward_iterator_tag)
{
    if(last <= first)return;
    const size_type n = mystl::distance(first, last);
    if(pos.cur == begin_.cur)
...
void deque<T>::reallocate_map_at_front(size_type need_buffer)
{
    ...
    // 更新数据
    map_allocator::deallocate(map_, map_size_);  // 没有destroy吗？
    map_ = new_map;
